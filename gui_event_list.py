...
class GuiEventListMixin:
    ...
    def _create_event_list_widgets(self, parent_frame):
        ...
        event_list_frame = ttk.LabelFrame(parent_frame, text="Event List", padding=10)
        ...
        on_select_cmd = getattr(self, 'on_event_select', lambda e: print("on_event_select not found"))
        on_double_click_cmd = getattr(self, 'on_event_double_click', lambda e: print("on_event_double_click not found"))
        ...
        select_all_cmd = getattr(self, 'select_all_events', lambda: print("select_all_events not found"))
        delete_selected_cmd = getattr(self, 'delete_selected_event', lambda: print("delete_selected_event not found"))
        add_delay_cmd = getattr(self, 'add_delay_to_event', lambda: print("add_delay_to_event not found"))
        delete_delay_cmd = getattr(self, 'delete_delay_events', lambda: print("delete_delay_events not found"))
        modify_delay_cmd = getattr(self, 'modify_delay_time', lambda: print("modify_delay_time not found"))
        move_up_cmd = getattr(self, 'move_event_up', lambda: print("move_event_up not found"))
        move_down_cmd = getattr(self, 'move_event_down', lambda: print("move_event_down not found"))

        ttk.Button(event_btn_frame, text="Select All", command=select_all_cmd).pack(side=tk.LEFT, padx=5)
        ttk.Button(event_btn_frame, text="Add Delay", command=add_delay_cmd).pack(side=tk.LEFT, padx=5)
        ttk.Button(event_btn_frame, text="Delete Delay", command=delete_delay_cmd).pack(side=tk.LEFT, padx=5)
        ttk.Button(event_btn_frame, text="Modify Delay", command=modify_delay_cmd).pack(side=tk.LEFT, padx=5)
        ttk.Button(event_btn_frame, text="↑", width=2, command=move_up_cmd).pack(side=tk.RIGHT, padx=2)
        ttk.Button(event_btn_frame, text="↓", width=2, command=move_down_cmd).pack(side=tk.RIGHT, padx=2)
        ...
        ttk.Button(random_btn_frame, text="Add Random Position", command=add_random_pos_cmd).pack(side=tk.LEFT, padx=5)
        ttk.Button(random_btn_frame, text="Add Random Delay", command=add_random_delay_cmd).pack(side=tk.LEFT, padx=5)
    ...
    def display_event(self, event, index):
        ...
        try:
            ...
            if event_type == 'keyboard':
                ...
                timestamp = f"{event.get('time', 0):>8.3f}s" # Timestamp (right-aligned, 8 spaces)
                display_str += f"{timestamp} K-{event_type_str:<4} {key}" # Type (left-aligned, 4 spaces)
            elif event_type == 'mouse_move':
                pos_x, pos_y = event.get('position', (0, 0))
                timestamp = f"{event.get('time', 0):>8.3f}s"
                pos_str = f"({pos_x:>4}, {pos_y:>4})" # Coordinates (right-aligned)
                ...
                event_type_str = event.get('event_type', 'click') # 'press' or 'release'
                pos_x, pos_y = event.get('position', (0, 0))
                timestamp = f"{event.get('time', 0):>8.3f}s"
                pos_str = f"({pos_x:>4}, {pos_y:>4})"
                display_str += f"{timestamp} M-{event_type_str:<6} {button:<5} {pos_str}" # Event type (left, 6), Button (left, 5)
                if 'random_range' in event:
                    range_px = event.get('random_range', 0)
                    display_str += f" +/-{range_px:<3}px"
            ...
            bg_color = None
            if event_type == 'delay':
                bg_color = '#E6F9E6' if 'random_range' in event else '#E6E6FF' # Light green for random, light blue for normal
            ...
            try:
                ...
                self.event_listbox.insert(tk.END, f"{index+1:<4} ! Error displaying event: {e}")
                self.event_listbox.itemconfig(tk.END, {'fg': 'red'})
            except: pass # Ignore if listbox is also broken
    ...
    def delete_selected_event(self):
        ...
        try:
            ...
            import traceback
            traceback.print_exc()
            messagebox.showerror("Error", f"An error occurred while deleting events: {e}")
            return
